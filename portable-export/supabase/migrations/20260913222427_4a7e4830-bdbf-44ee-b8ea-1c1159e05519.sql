...
DECLARE
  ...
  v_url text := coalesce(nullif(current_setting('app.settings.dispatch_url', true), ''), 'https://tudominio.com/api/public/push/dispatch');
BEGIN
...
DECLARE
  ...
  v_anon_key text := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVvZ2x4d3RyaXRjc3JnbHdpbWVqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM1MzY1NjgsImV4cCI6MjA5OTExMjU2OH0._rVbxQ9JDgB5FFMVxaGgSKFTi-ozyzrlOvsEgHvJ-3o';
  v_dispatch_url text := coalesce(nullif(current_setting('app.settings.dispatch_url', true), ''), 'https://tudominio.com/api/public/push/dispatch');
BEGIN
