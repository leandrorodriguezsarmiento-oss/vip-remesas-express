...
const WHATSAPP_GROUP_URL =
  "https://chat.whatsapp.com/IJPFYGrhrc4JAddkp1ohnI?s=cl&p=a&ilr=0&amv=3";
  ...
  head: () => ({
    meta: [
      { title: "Configuración de tu cuenta | VIP Remesas" },
      {
        name: "description",
        content:
          "Edita tu perfil, foto, idioma, contactos guardados y verifica tu cuenta VIP Remesas.",
      },
      {
        property: "og:title",
        content: "Configuración de tu cuenta | VIP Remesas",
      },
      {
        property: "og:description",
        content:
          "Perfil, foto, idioma, contactos y verificación de tu cuenta VIP Remesas.",
      },
  ...
  const profile = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "id, full_name, phone, username, cpf, country, avatar_url, verified, preferred_language, province",
        )
        .eq("id", user.id)
        .maybeSingle();
    ...
    <div className="space-y-5">
      <div className="animate-rise">
        ...
        <p className="mt-1 text-sm font-semibold text-muted-foreground">
          Tu perfil, contactos, historial e idioma.
        </p>
      </div>
      ...
      {profile.data && (
        <VerificationCard verified={profile.data.verified} />
      )}
...
function ProfileCard({
  ...
  async function uploadAvatar(file: File) {
    if (file.size > 4 * 1024 * 1024) {
      return toast.error("La imagen no puede pasar de 4 MB");
    }
  ...
  return (
    ...
    >
      <p className="mb-3 text-xs font-semibold text-muted-foreground">
        Por seguridad, la contraseña actual no se puede mostrar.
        Escribe una nueva y usa el ojo para verla.
      </p>
...
function VerificationCard({
  ...
  const m = useMutation({
    ...
    onSuccess: () =>
      toast.success(
        "Solicitud enviada. Te avisaremos al verificar.",
      ),
      ...
      {verified ? (
        ...
        <>
          <p className="mb-3 text-sm text-muted-foreground">
            Verifica tu cuenta para subir tus límites de envío.
            Nuestro equipo revisa tus datos.
          </p>
    ...
    queryFn: async () => {
      ...
      return (data ?? []).filter((r) => {
        const key = `${(r.full_name ?? "")
          .trim()
          .toLowerCase()}|${(r.phone ?? "").replace(/\D/g, "")}`;
    ...
    >
      <ul className="space-y-2">
        {list.data?.map((r) => (
          <li
            key={r.id}
            className="flex items-center gap-2 rounded-lg border border-border bg-background/60 p-3"
          >
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[11px] text-muted-foreground">
                ...
                {(r as { address?: string | null }).address
                  ? ` · ${
                      (r as {
                        address?: string | null;
                      }).address
 ...
 *
 * IMPORTANTE:
 * Un error de MFA nunca debe romper la pantalla de configuración.
 */
function TwoFactorCard() {
  ...
  const verified = (factors.data ?? []).some(
    (f) => f.status === "verified",
  );
  ...
  async function start() {
    ...
    try {
      ...
      toast.error(
        e instanceof Error
          ? e.message
          : "No se pudo activar el doble factor",
      );
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "No se pudo desactivar el doble factor",
      );
    }
  }
      ...
      {verified ? (
        ...
        <div className="space-y-3">
          <p className="text-xs font-semibold text-muted-foreground">
            Escanea el código con Google Authenticator y escribe
            los 6 dígitos.
          </p>

          <img
            src={qr}
            alt="Código QR de doble factor"
