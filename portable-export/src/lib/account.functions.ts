...
export const resolveLoginIdentifier = createServerFn({ method: "POST" }).inputValidator((input: { identifier: string }) => z.object({ identifier: z.string().trim().min(3).max(255) }).parse(input)).handler(async ({ data }) => {
  ...
  if (isEmailLike(data.identifier)) {
    ...
    const { data: row } = await supabaseAdmin.from("login_aliases").select("auth_email").ilike("alias", email).maybeSingle();
    return { email: (row?.auth_email as string | undefined) ?? email };
  }
  const candidates = Array.from(new Set([normalizeAlias("username", data.identifier), normalizeAlias("phone", data.identifier)].filter((v) => v.length >= 3)));
  for (const alias of candidates) {
    const { data: row } = await supabaseAdmin.from("login_aliases").select("auth_email").ilike("alias", alias).maybeSingle();
...
export const updateMyAliases = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: unknown) => z.object({ username: z.string().trim().max(24).optional(), phone: z.string().trim().max(24).optional(), cpf: z.string().trim().max(20).optional() }).parse(input)).handler(async ({ data, context }) => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data: existing } = await supabaseAdmin.from("login_aliases").select("auth_email").eq("user_id", context.userId).limit(1).maybeSingle();
  const authEmail = (existing?.auth_email as string | undefined) ?? (context.claims as { email?: string } | null)?.email;
  if (!authEmail) throw new Error("Cuenta sin identificador base");
  const rows = [data.username ? { alias: normalizeAlias("username", data.username), kind: "username" } : null, data.phone ? { alias: normalizeAlias("phone", data.phone), kind: "phone" } : null, data.cpf ? { alias: normalizeAlias("cpf", data.cpf), kind: "cpf" } : null].filter((r): r is { alias: string; kind: string } => !!r && r.alias.length >= 3);
  for (const r of rows) { const { data: taken } = await supabaseAdmin.from("login_aliases").select("id, user_id").ilike("alias", r.alias).maybeSingle(); if (taken && taken.user_id !== context.userId) throw new Error(`El identificador ${r.alias} ya está en uso`); if (!taken) await supabaseAdmin.from("login_aliases").insert({ alias: r.alias, kind: r.kind, auth_email: authEmail, user_id: context.userId }); }
  return { ok: true };
});
