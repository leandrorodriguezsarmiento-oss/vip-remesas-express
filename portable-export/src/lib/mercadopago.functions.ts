...
async function fetchMercadoPago(url: string, init: RequestInit): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error("Mercado Pago no respondió en 6 segundos. La API está tardando o el Access Token no está respondiendo.");
    }
    throw error;
  } finally {
...
async function resolveMercadoPagoPayment(accessToken: string, order: MercadoPagoOrder): Promise<{
  ...
  // Orders API puede crear la order de forma asíncrona y devolverla inicialmente sin los datos PIX.
  // Hacemos SOLO una consulta adicional para no dejar el botón bloqueado durante decenas de segundos.
  if (current.id) {
    await new Promise((resolve) => setTimeout(resolve, 800));
    current = await getMercadoPagoOrder(accessToken, current.id);
    const updatedPayment = current.transactions?.payments?.[0];
    const updatedMethod = updatedPayment?.payment_method;
    const pixCode = updatedMethod?.qr_code?.trim() || null;
    if (pixCode) {
      return {
        checkoutUrl: updatedMethod?.ticket_url?.trim() || null,
        pixCode,
        qrCodeBase64: updatedMethod?.qr_code_base64?.trim() || null,
        paymentId: updatedPayment?.id ?? null,
        status: updatedPayment?.status ?? null,
        statusDetail: updatedPayment?.status_detail ?? null,
      };
    }
  }
...
export const createMercadoPagoPreference = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: tx, error } = await supabaseAdmin.from("transactions")
      .select("id,user_id,tracking_id,total_brl,recipient_name,status")
      .eq("id", data.transactionId).maybeSingle();
    ...
    const { data: existing } = await supabaseAdmin.from("mercadopago_payments")
      .select("order_id,checkout_url,qr_code")
      .eq("transaction_id", tx.id).eq("internal_status", "created")
      .order("created_at", { ascending: false }).limit(1).maybeSingle();

    if (existing?.order_id && existing.qr_code) {
      return { preferenceId: existing.order_id, checkoutUrl: existing.checkout_url ?? null, pixCode: existing.qr_code, qrCodeBase64: null };
    }
    ...
    const { error: txUpdateError } = await supabaseAdmin.from("transactions")
      .update({ payment_method: "mercadopago", notes: `mp_order:${json.id}` }).eq("id", tx.id);
    if (txUpdateError) throw txUpdateError;

    const { error: insertError } = await supabaseAdmin.from("mercadopago_payments").insert({
      transaction_id: tx.id, user_id: tx.user_id, tracking_id: tx.tracking_id,
      order_id: json.id, preference_id: json.id,
      checkout_url: resolved.checkoutUrl, qr_code: resolved.pixCode,
      ...
      internal_status: "created", amount, currency: "BRL",
    });
