...
async function resolveMercadoPagoPayment(accessToken: string, order: MercadoPagoOrder): Promise<{
  ...
  const statusDetail = payment?.status_detail ?? "desconocido";
  throw new Error(
    `Mercado Pago creó la Order ${current.id ?? ""}, pero todavía no devolvió qr_code PIX. Estado: ${status}; detalle: ${statusDetail}.`,
  );
}
