...
export const createMercadoPagoPreference = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabaseAdmin.from("transactions").update({ payment_method: "mercadopago", notes: `mp_order:${json.id}` }).eq("id", tx.id);
    ...
    return { preferenceId: json.id, checkoutUrl, pixCode, qrCodeBase64: payment?.payment_method?.qr_code_base64 ?? null };
  });
