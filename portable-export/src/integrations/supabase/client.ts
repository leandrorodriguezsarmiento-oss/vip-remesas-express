...
// PRODUCCIÓN: este cliente debe usar SIEMPRE el proyecto Supabase actual.
// No permitir que variables de entorno antiguas cambien el proyecto en el navegador.
...
function loadGoogleIdentityScript(): Promise<void> {
  if (typeof window === 'undefined') {
    return Promise.reject(
      new Error('Google Sign-In solo está disponible en el navegador.'),
    );
  }
...
function readableGoogleError(error: unknown): Error {
  ...
  return new Error(
    parts.join(' | ') || 'No se pudo iniciar con Google.',
  );
}
    ...
    const handleCredential = async (
      ...
      try {
        ...
        const signInResult = await Promise.race([
          ...
          new Promise<never>((_, timeoutReject) => {
            window.setTimeout(() => {
              timeoutReject(
                new Error(
                  'Google inició correctamente, pero Supabase no respondió a tiempo.',
                ),
              );
        ...
        if (signInResult.error) {
          throw readableGoogleError(signInResult.error);
        }
        ...
        console.error(
          'Google ID token authentication error:',
          readable,
        );
...
function createSupabaseClient() {
  // IMPORTANTE:
  // NO usar import.meta.env aquí.
  // El navegador debe quedar fijado al proyecto Supabase actual.
  //
  // Esto evita que una variable de entorno antigua,
  // un build anterior o una configuración de Lovable
  // haga que Google envíe el token al proyecto viejo.
  ...
  const originalSignInWithIdToken =
    originalAuth.signInWithIdToken.bind(
      originalAuth,
    );
  ...
  const authProxy = new Proxy(originalAuth, {
    get(target, prop, receiver) {
      ...
      if (prop === 'signInWithIdToken') {
        return originalSignInWithIdToken;
      }
