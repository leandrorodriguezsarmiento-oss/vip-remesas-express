...
export function MfaGate({ userId, email, children }: {
  userId: string;
  ...
  // Important: a staff member without TOTP must be able to reach Settings
  // to enroll it. Once TOTP exists, Settings is protected by AAL2 like the panel.
    ...
    <div className="grid min-h-screen place-items-center bg-gradient-vip px-5">
      <div className="w-full max-w-sm rounded-2xl border border-gold/40 bg-card p-6 shadow-glow">
        ...
        {mode === "checking" ? (
          ...
          <>
            <h1 className="flex items-center gap-2 font-display text-xl font-extrabold"><ShieldCheck className="h-5 w-5 text-gold" />Verificación de staff</h1>
            <p className="mt-2 text-xs font-bold text-muted-foreground">Escribe los 6 dígitos de tu app de autenticación para abrir el panel.</p>
            <input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoFocus placeholder="000000" className="mt-4 w-full rounded-lg border border-border bg-background px-3 py-3 text-center text-2xl font-extrabold tracking-[0.4em] outline-none focus:border-gold" />
            <button onClick={() => void submitTotp()} disabled={busy || code.length !== 6} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-gold px-4 py-3 text-sm font-extrabold text-primary-foreground shadow-gold disabled:opacity-60">{busy && <Loader2 className="h-4 w-4 animate-spin" />}Desbloquear</button>
            <button onClick={() => void signOut()} className="mt-4 flex w-full items-center justify-center gap-2 text-xs font-bold text-destructive"><LogOut className="h-3.5 w-3.5" /> Salir de la cuenta</button>
          </>
        )}
      </div>
    </div>
