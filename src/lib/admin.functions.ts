...
export const updateTransactionWorkflow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<WorkflowResult> => {
   try {
    ...
    if (!isAdmin) {
      const { data: canRemesas, error: permissionError } = await context.supabase.rpc("has_organizer_permission", {
...
export const setOrganizerRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.enabled) {
      ...
      // Los nuevos organizadores empiezan SIN permisos. El admin los selecciona en /organizer-permissions.
...
export const deleteUserAsAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // Limpieza explícita de roles/permisos antes de Auth para evitar bloqueos por relaciones de usuario.
