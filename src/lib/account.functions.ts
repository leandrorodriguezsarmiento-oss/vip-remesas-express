...
export const updateMyAliases = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: unknown) => z.object({ username: z.string().trim().max(24).optional(), phone: z.string().trim().max(24).optional(), cpf: z.string().trim().max(20).optional() }).parse(input)).handler(async ({ data, context }) => {
  ...
  for (const r of rows) {
    const { data: taken } = await supabaseAdmin.from("login_aliases").select("id, user_id").ilike("alias", r.alias).maybeSingle();
    if (taken && taken.user_id !== context.userId) throw new Error(`El identificador ${r.alias} ya está en uso`);
    if (!taken) await supabaseAdmin.from("login_aliases").insert({ alias: r.alias, kind: r.kind, auth_email: authEmail, user_id: context.userId });
  }
  return { ok: true };
});
